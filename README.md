...
Для раздачи статических файлов следует использовать Anycast.

Для запросов, которые требуют работу с базами данных, сделать по поддомену для каждого ЦОДа.

## Схема балансировки

При начале пользовательской сессии клиент делает запрос на сервер балансировщика.
Балансировщик возвращает URL сервера, на который клиент должен делать запрос.
Далее все запросы идут на такой сервер.

## Механизм регулировки трафика между ЦОД

Балансировщик собирает информацию о загруженности ЦОД-ов, и в зависимости от этого принимает решение об определении запроса на конкретный ЦОД.
