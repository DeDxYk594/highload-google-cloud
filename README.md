...
Приложение будет разворачиваться на BareMetal-серверах (без применения managed-решений). Это необходимо для:

- Уменьшения затрат
- Гибкости в выборе хостинга
- Снижения рисков

Выпишем пиковые нагрузочные показатели сервисов:

| Сервис            | RPS(max)   |
| ----------------- | ---------- |
| upload-server     | 12880      |
| download-server   | 11040      |
| archive-worker    | 3680       |
| inference-indexer | 16000      |
| media-worker      | 22080      |
| mediaserver       | 276000     |
| search            | 18400      |
| cloud-features    | 1840       |
| cloud-explorer    | 92000      |
| **Итого**         | **539120** |

Все сервисы должны быть разработаны на C++. Распределение ресурсов - через Kubernetes.

### Stateless-сервисы

Деплой - через Kubernetes

| Сервис          | CPU count | RAM GB |
| --------------- | --------- | ------ |
| upload-server   | 190       | 190    |
| download-server | 320       | 320    |
| archive-worker  | 72        | 72     |
| media-worker    | 220       | 220    |
| search          | 870       | 870    |
| cloud-features  | 80        | 80     |
| cloud-explorer  | 130       | 130    |
...
Требуется 2760 ядер CPU и 5520 GB RAM.

Медиасерверы разворачивать на Bare Metal серверах с помощью Ansible.
...
| Кластер       | read RPS (peak) | write RPS (peak) | CPU count | RAM GB | SSD TB | HDD TB |
| ------------- | --------------- | ---------------- | --------- | ------ | ------ | ------ |
| YDB           | 100000          | 10000            | 2000      | 4000   | -      | 30 Пб  |
| ElasticSearch | 18400           | 10000            | 300       | 2000   | -      | 1 Эб   |
| Redis         | 276000          | 30000            | 200       | 500    | 10 Тб  | -      |
| CEPH          | 11040           | 12880            | 1000      | 8000   | -      | 12 Эб  |

Конфигурация сервера для ElasticSearch и CEPH:

- AMD EPYC 9554P
- 256 GB RAM
- 24 x 18 TB HDD
- 4 x полка на 12 HDD x 18 ТБ HDD

Требуется 10000 таких серверов

Конфигурация сервера для Redis:
- AMD EPYC 9654
- 1024 GB RAM
- 4 TB SSD

Требуется 15 таких серверов

Конфигурация сервера для YDB такая же, как для CEPH, только без полок с HDD. Потребуется 40 таких серверов.

### Инференс моделей

Для ASR используется модель Whisper

Для OCR используется PaddleOCR

| Модель    | Объём информации / сек | GB RAM | CPU ядер | GPU (Nvidia A100) |
| --------- | ---------------------- | ------ | -------- | ----------------- |
| Whisper   | 100 минут аудио        | 400    | 24       | 10 карт           |
| PaddleOCR | 50 страниц A4          | 400    | 24       | 10 карт           |
...
[^4]: Результаты опроса, проведённого автором среди 18 человек: https://docs.google.com/spreadsheets/d/1Ivwkm36-tmMWPxk_14mbQxpYs6oTPlgk/edit (дата обращения: 23.02.2025)
